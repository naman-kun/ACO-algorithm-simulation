...
function SlideViewer({ title, slides, credits }: SlideViewerProps) {
    ...
    // Preload only current and adjacent slides (prev + next)
    useEffect(() => {
        if (!hasSlides) return;
        ...
        // Set loading state for current slide
        if (!loadedImages.has(currentSlide) && !imageErrors.has(currentSlide)) {
            setIsLoading(true);
        } else {
            setIsLoading(false);
        }
    }, [currentSlide, slides, hasSlides, loadedImages, imageErrors]);
    ...
    return (
        <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden mb-8 group">
            ...
            <div className="relative w-full aspect-[16/9] bg-black/90 flex items-center justify-center overflow-hidden">
                {/* Loading State */}
                {isLoading && !imageErrors.has(currentSlide) && (
                ...
                {imageErrors.has(currentSlide) && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground p-8 text-center">
                        <ImageOff className="w-12 h-12 mb-4 opacity-30" />
                        <p className="text-sm font-mono opacity-60">Failed to load slide</p>
                    </div>
                )}
                ...
                <img
                    ...
                    className={`max-h-full max-w-full object-contain transition-opacity duration-200 ${
                        isLoading || imageErrors.has(currentSlide) ? 'opacity-0' : 'opacity-100'
                    }`}
                    loading="lazy"
                />
